...
pub struct MatchPattern {
    pub(crate) internal: *mut gumjs_sys::GumMatchPattern,
}
...
impl MatchPattern {
    pub fn from_string(pattern: &str) -> Option<Self> {
        ...
        let internal =
            unsafe { gumjs_sys::gum_match_pattern_new_from_string(pattern.as_ptr().cast()) };
        if !internal.is_null() {
            Some(Self { internal })
        } else {
            None
...
impl Drop for MatchPattern {
    fn drop(&mut self) {
        unsafe { gumjs_sys::gum_match_pattern_unref(self.internal) }
    }
}
...
pub struct MemoryRange {
    pub(crate) memory_range: gumjs_sys::GumMemoryRange,
}

impl MemoryRange {
    pub(crate) fn from_raw(memory_range: *const gumjs_sys::GumMemoryRange) -> MemoryRange {
        MemoryRange {
    ...
    pub fn new(base_address: NativePointer, size: usize) -> MemoryRange {
        MemoryRange {
            memory_range: gumjs_sys::GumMemoryRange {
                base_address: base_address.0 as GumAddress,
                size: size as _,
            },
        }
    }
    ...
    pub fn scan(&self, pattern: &MatchPattern) -> Vec<ScanResult> {
        ...
        unsafe {
            ...
            extern "C" fn callback32(address: u64, size: u32, user_data: *mut c_void) -> i32 {
                callback64(address, size, user_data)
            }

            extern "C" fn callback64(address: GumAddress, size: gsize, user_data: gpointer) -> i32 {
            ...
            gumjs_sys::gum_memory_scan(
                &self.memory_range as *const gumjs_sys::GumMemoryRange,
                pattern.internal,
                Some(callback),
