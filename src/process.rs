...
pub fn enumerate_threads() -> Vec<ThreadDetailsOwned> {
    ...
    unsafe {
        frida_gumjs_rs_sys::gum_process_enumerate_threads(
            Some(callback),
...
pub fn enumerate_modules() -> Vec<ModuleDetailsOwned> {
    ...
    unsafe {
        frida_gumjs_rs_sys::gum_process_enumerate_modules(
            Some(callback),
...
pub fn id() -> usize {
    unsafe { frida_gumjs_rs_sys::gum_process_get_id() as usize }
}

pub fn get_current_thread_id() -> usize {
    unsafe { frida_gumjs_rs_sys::gum_process_get_current_thread_id() as usize }
}

pub fn is_debugger_attached() -> bool {
    unsafe { frida_gumjs_rs_sys::gum_process_is_debugger_attached() != 0 }
}

pub fn get_current_dir() -> String {
    NativePointer(unsafe { frida_gumjs_rs_sys::_frida_g_get_current_dir() as *mut _ }).try_into().unwrap_or_default()
}

pub fn get_home_dir() -> String {
    NativePointer(unsafe { frida_gumjs_rs_sys::_frida_g_get_home_dir() as *mut _ }).try_into().unwrap_or_default()
}

pub fn get_tmp_dir() -> String {
    NativePointer(unsafe { frida_gumjs_rs_sys::_frida_g_get_tmp_dir() as *mut _ }).try_into().unwrap_or_default()
}

pub fn get_main_module() -> ModuleDetailsOwned {
    let details = unsafe { frida_gumjs_rs_sys::gum_process_get_main_module() };
...
pub fn find_module_by_name(name:&str)->Option<ModuleDetailsOwned>{
    ...
    for module in modules {
        ...
        if cmp_name.ends_with(".so")==false{
...
pub fn find_base_address(module_name: &str) -> NativePointer {
    ...
    unsafe {
        NativePointer(
            frida_gumjs_rs_sys::gum_module_find_base_address(module_name.as_ptr().cast()) as *mut c_void,
        )
    }
}
