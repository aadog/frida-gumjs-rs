...
unsafe extern "C" fn call_on_message<T:ScriptMessageHandler>(
    message: *const gchar,
    _data: *mut frida_gumjs_rs_sys::GBytes,
    ...
    if !_data.is_null(){
        ...
        let d=frida_gumjs_rs_sys::_frida_g_bytes_get_data(_data,&mut d_size);
        if d_size>0{
...
pub struct Script {
    inner: *mut frida_gumjs_rs_sys::GumScript,
}

impl GObject for Script {
    fn un_ref(&self) {
        unsafe { frida_gumjs_rs_sys::_frida_g_object_unref(self.inner.cast()) };
    }
}
...
impl Script {
    pub fn v8()->*mut GumScriptBackend{
        unsafe{
            frida_gumjs_rs_sys::gum_script_backend_obtain_v8()
        }
    }
    pub fn qjs()->*mut GumScriptBackend{
        unsafe{
            frida_gumjs_rs_sys::gum_script_backend_obtain_qjs()
        }
    }
    pub fn create_qjs_script(backend:*mut frida_gumjs_rs_sys::GumScriptBackend,name: &str, source: &str) -> Result<Script, Box<dyn std::error::Error>>{
        Self::create_script(backend,name,source)
    }
    pub fn create_v8_script(backend:*mut frida_gumjs_rs_sys::GumScriptBackend,name: &str, source: &str) -> Result<Script, Box<dyn std::error::Error>>{
        Self::create_script(backend,name,source)
    }
    pub fn create_script(eng:*mut GumScriptBackend,name: &str, source: &str) -> Result<Script, Box<dyn std::error::Error>> {
        ...
        backend = unsafe { frida_gumjs_rs_sys::gum_script_backend_obtain_qjs() };

        let mut error: *mut frida_gumjs_rs_sys::GError = null_mut();
        let c_name = CString::new(name).unwrap();
        let c_source = CString::new(source).unwrap();

        let gum_script = unsafe {
            frida_gumjs_rs_sys::gum_script_backend_create_sync(
                backend,
                c_name.as_ptr() as _,
                c_source.as_ptr() as _,
                null_mut(),
                null_mut(),
                &mut error,
            )
        ...
        if !error.is_null() {
            ...
            unsafe { frida_gumjs_rs_sys::_frida_g_error_free(error) };
            bail!(message);
        } else {
            Ok(Script {
                inner: gum_script,
    ...
    pub fn load(&self) {
        unsafe { frida_gumjs_rs_sys::gum_script_load_sync(self.inner, null_mut()) };
    }
    pub fn unload(&self) {
        unsafe { frida_gumjs_rs_sys::gum_script_unload_sync(self.inner, null_mut()) };
    }
    // pub fn post(&self){
    //     unsafe{frida_gumjs_rs_sys::gum_script}
    // }
    pub fn on_message<T:ScriptMessageHandler>(&self, handler: &T) {
        unsafe {
            ...
            frida_gumjs_rs_sys::gum_script_set_message_handler(
                self.inner,
                callback,
    ...
    pub fn post(&self,message:&str){
        let c_name = CString::new(message).unwrap();
        unsafe{
            frida_gumjs_rs_sys::gum_script_post(self.inner,c_name.as_ptr(),null_mut());
        }
    }
    pub fn on_debug_message<T:ScriptDebugMessageHandler>(&self, handler: &T) {
        unsafe {
            ...
            frida_gumjs_rs_sys::gum_script_set_debug_message_handler(
                self.inner,
                callback,
                handler as *const _ as *mut c_void,
                None,
